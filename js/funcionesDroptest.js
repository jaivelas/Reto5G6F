...
function postMachine (){
    ...
    let cajas = {
        ...
        category:{id: +$("#select-categoria1").val()}
    };
...
function pintarMachine(json_maquinas){
    ...
    for (i=0;i<json_maquinas.length;i++){
        ...
        myTable+="<td> <button onclick='deleteMachine("+json_maquinas[i].id+")'> Borrar</button>";
        myTable+="<td> <button onclick='putMachine("+json_maquinas[i].id+")'> Actualizar</button>";
        myTable+="</tr>";
    }
    myTable+="</table>";
